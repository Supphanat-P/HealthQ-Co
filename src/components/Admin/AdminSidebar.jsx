...
const AdminSidebar = () => {
    ...
    const navItems = [
        { id: "dashboard", icon: LayoutDashboard, label: "แดชบอร์ด", path: "/admindashboard" },
        { id: "appointments", icon: Calendar, label: "จัดการนัดหมาย", path: "/adminappointments" },
        { id: "patients", icon: Users, label: "จัดการผู้ป่วย", path: "/adminpatients" }
    ];
                        ...
                        return (
                            <li key={item.id}>
                                ...
                                >
                                    {({ isActive }) => (
                                        <>
                                            <div className={`p-2.5 rounded-xl mr-4 transition-all duration-300 ${isActive
                                                ? "bg-white/20 shadow-md"
                                                : "bg-gray-100 hover:bg-blue-100"
                                                }`}>

                                                <Icon className={`w-5 h-5 ${isActive ? "text-white" : "text-blue-900"}`} />
                                            </div>
                                            &nbsp;
                                            <span className={`font-medium text-navy ${isActive ? "text-white font-semibold" : ""}`}>
                                                {item.label}
                                            </span>
                                        </>
                                    )}
